...
{
	A_routineHead p = (A_routineHead)checked_malloc(sizeof(*p));
	p->pos = pos;
...
A_dec A_TypeDec(A_pos pos, S_symbol name, A_type type)
{
	A_dec p = (A_dec)checked_malloc(sizeof(*p));
	p->kind = A_typeDec;
	p->pos = pos;
...
A_routinePart A_RoutinePart(A_routinePartHead head, A_routine subRoutine){
	A_routinePart p = (A_routinePart)checked_malloc(sizeof(*p));
	p->head = head;
